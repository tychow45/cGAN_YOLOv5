...
from operations import Conv, ReLU, ReLU6, LeakyReLU, AvgPool, BatchNorm
...
class GAN():
    ...
    def Layer(self, input_):
        ...
        with tf.variable_scope("Composite"):
            ...
            next_layer = tfa.activations.mish(next_layer)
    ...
    #def generator(self, input_):
        ...
        for i in range(1, 6):
            input_ = self.DenseBlock(input_, name = 'Encoder' + str(i), layers = 4) # 5 DB
            collect_conv.append(input_) # Concat
            input_ = self.TransitionDown(input_, name = 'TD' + str(i)) # 5 TD

        input_ = self.DenseBlock(input_, name = 'BottleNeck', layers = 15)

        for i in range(1, 6): 
            input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU' + str(6 - i)) # 5 TU
            input_ = tf.concat([input_, collect_conv[6 - i - 1]], axis = 3, name = 'Decoder' + str(6 - i) + '/Concat') # Concat
            input_ = self.DenseBlock(input_, name = 'Decoder' + str(6 - i), layers = 4) # 5 DB
    ...
    def generator(self, input_):
        """
        FC-DenseNet103 (103 Layer Tiramisu)
        """
        #Input convolution
        with tf.variable_scope('InputConv') as scope:
            input_ = Conv(input_, kernel_size = 3, stride = 1, output_channels= self.growth_rate * 4)
        ...
        #Encoder side
        input_ = self.DenseBlock(input_, name = 'Encoder1', layers = 4)
        collect_conv.append(input_)
        input_ = self.TransitionDown(input_, name = 'TD1')

        input_ = self.DenseBlock(input_, name = 'Encoder2', layers = 5)
        collect_conv.append(input_)
        input_ = self.TransitionDown(input_, name = 'TD2')

        input_ = self.DenseBlock(input_, name = 'Encoder3', layers = 7)
        collect_conv.append(input_)
        input_ = self.TransitionDown(input_, name = 'TD3')

        input_ = self.DenseBlock(input_, name = 'Encoder4', layers = 10)
        collect_conv.append(input_)
        input_ = self.TransitionDown(input_, name = 'TD4')

        input_ = self.DenseBlock(input_, name = 'Encoder5', layers = 12)
        collect_conv.append(input_)
        input_ = self.TransitionDown(input_, name = 'TD5')

        #Bottleneck layer
        input_ = self.DenseBlock(input_, name = 'BottleNeck', layers = 15)

        #Decoder side
        input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU1')
        input_ = tf.concat([input_, collect_conv[4]], axis = 3, name = 'Decoder1' + '/Concat')
        input_ = self.DenseBlock(input_, name = 'Decoder1', layers = 12)
        
        input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU2')
        input_ = tf.concat([input_, collect_conv[3]], axis = 3, name = 'Decoder2' + '/Concat')
        input_ = self.DenseBlock(input_, name = 'Decoder2', layers = 10)

        input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU3')
        input_ = tf.concat([input_, collect_conv[2]], axis = 3, name = 'Decoder3' + '/Concat')
        input_ = self.DenseBlock(input_, name = 'Decoder3', layers = 7)

        input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU4')
        input_ = tf.concat([input_, collect_conv[1]], axis = 3, name = 'Decoder4' + '/Concat')
        input_ = self.DenseBlock(input_, name = 'Decoder4', layers = 5)

        input_ = self.TransitionUp(input_, output_channels = self.growth_rate * 4, name = 'TU5')
        input_ = tf.concat([input_, collect_conv[0]], axis = 3, name = 'Decoder5' + '/Concat')
        input_ = self.DenseBlock(input_, name = 'Decoder5', layers = 4)
    ...
    def discriminator(self, input_, target, stride = 2, layer_count = 4):
        ...
        for i, output_channels in enumerate(layer_specs, 1):
            ...
            with tf.variable_scope('Layer' + str(i)) as scope:
                ...
                input_ = tfa.activations.mish(input_)
    ...
    def build_model(self):
        ...
        with tf.variable_scope('Placeholders') as scope:
            self.RealA = tf.placeholder(name='A', shape=[None, 224, 224, 3], dtype=tf.float32)
            self.RealB = tf.placeholder(name='B', shape=[None, 224, 224, 3], dtype=tf.float32)
    ...
    def train(self):
        ...
        if self.custom_data:
            ...
            data = sorted(os.listdir(self.A_dir)) # Hazy image
            ...
            self.A_train = np.zeros((total_image_count, 224, 224, 3)) # Hazy training data
            self.B_train = np.zeros((total_image_count, 224, 224, 3)) # GT training data
            self.A_val = np.zeros((val_image_count, 224, 224, 3)) # Hazy validation data
            self.B_val = np.zeros((val_image_count, 224, 224, 3)) # GT validation data

            print(self.A_train.shape, self.A_val.shape)

            for i, file in enumerate(train_data):
                self.A_train[i] = cv2.imread(os.path.join(os.getcwd(), self.A_dir, file), 1).astype(np.float32) # Hazy training data
                self.B_train[i] = cv2.imread(os.path.join(os.getcwd(), self.B_dir, file), 1).astype(np.float32) # GT training data

            for i, file in enumerate(val_data):
                self.A_val[i] = cv2.imread(os.path.join(os.getcwd(), self.A_dir, file), 1).astype(np.float32) # Hazy validation data
                self.B_val[i] = cv2.imread(os.path.join(os.getcwd(), self.B_dir, file), 1).astype(np.float32) # GT validation data
    ...
    def inference(self, input_dir, result_dir):
        ...
        print('Time for inferencing: ', format(time.time() - begin, '.3f'))
